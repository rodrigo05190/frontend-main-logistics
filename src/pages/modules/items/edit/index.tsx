...
interface EditSubmoduleProps {
  submodule: any
}

const EditSubmodule: React.FC<EditSubmoduleProps> = (props) => {
  const {
    handleSubmit,
    register,
    formState: { errors },
    ...
    defaultValues: {
      ...
      description: props.submodule.description,
    },
  })
  ...
  const [fields, setFields] = useState<{ fieldType: string; label: string; items?: string[] }[]>(
    props.submodule.fields
  )
  const [loading, setLoading] = useState(false)

  const onSubmit = async (data: Form) => {
    const response = await SubmoduleRepository.update(props.submodule.id, {
      ...data,
      fields,
    })

    if (response.status === 200) {
      triggerAlert('success', 'Submódulo atualizado com sucesso!')
      closeModal()
    } else {
      triggerAlert('error', 'Ocorreu um erro ao atualizar o submódulo.')
    }
  }
      ...
      >
        ...
        <Box flexDirection="column" width="100%" border="1px dashed #000">
          {fields.map((field, index) => (
            <Box key={index} flexDirection="column" width="100%">
              <Box align="end" flexDirection="row" gap="16px" padding="0" width="100%">
                ...
                />
                {index > props.submodule.fields.length - 1 && (
          ...
          <Button
            ...
            width="200px"
            onClick={() =>
              setFields([...fields, { fieldType: 'input', label: 'Título de Pergunta' }])
            }
          >
